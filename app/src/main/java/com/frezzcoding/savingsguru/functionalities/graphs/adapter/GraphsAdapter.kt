...
class GraphsAdapter(private val listener: OnClickListenerSavings) :
    ListAdapter<EstimatedSavings, GraphViewHolder>(GraphsDiffUtil()) {

    class GraphsDiffUtil : DiffUtil.ItemCallback<EstimatedSavings>() {
        override fun areItemsTheSame(
            oldItem: EstimatedSavings,
            newItem: EstimatedSavings
        ): Boolean {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): GraphViewHolder {
        ...
        return GraphViewHolder(
            DataBindingUtil.inflate(
                inflater,
                R.layout.item_estimated_savings,
                parent,
                false
            ), listener
        )
